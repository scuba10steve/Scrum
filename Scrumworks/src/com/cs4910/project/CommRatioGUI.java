...
public class CommRatioGUI extends JPanel { 
  ...
  final Border mainBorderType = new MatteBorder(2,0,0,0,Color.BLACK);
    ...
    totalRatioCompleted = 49; 
    lowRatioCompleted = 24; 
    highRatioCompleted = 76; 
    projectName = "Commerce's Project" ; 
    releaseName = "Commerce's Release 42" ; 
  ...
  public class InfoPanel extends JPanel {
    public InfoPanel() { 
      ...
      basicInfoPanel.setBorder( new TitledBorder(mainBorderType,"COMMITMENT RATIO REPORT", 0, 0, big)); 
      ...
      velocityPanel.setBorder(new TitledBorder(mainBorderType,"COMMITMENT RATIO", 0, 0, notAsBig));
      ...
      inVelocityPanel.add(new JLabel(totalRatioCompleted + "% of the points committed to"));     
      ...
      JLabel lbl = new JLabel("Compares story points completed with the points committed to"); 
      lbl.setFont(norm); 

      JPanel yetAnotherPanel = new JPanel( new GridLayout(1,2)); 
      yetAnotherPanel.add(inVelocityPanel); 
      JPanel spacePanel = new JPanel(); 
      //spacePanel.setBorder(new EmptyBorder(100,100,0,0));
    ...
    int VERT_OFFSET = 60; 
      ...
      setBorder(new TitledBorder(mainBorderType,"RELEASE FORECAST", 0, 0, notAsBig)); 
			completedLinePoints = new ArrayList<Point>(); 
      ...
      committedLinePoints = new ArrayList<Point>(); 
      //get points from another class
      ...
      maxY = vertStep * 10;     
        ...
        g.drawLine( getJavaX(i), VERT_OFFSET , getJavaX(i), getHeight() - VERT_OFFSET ) ; 
        g.setColor( Color.BLACK ) ; 
        g.drawString( "" + i , getJavaX(i) - 5, getHeight() - VERT_OFFSET + 18);
      }
      ...
      for (int i = vertStep; i <= maxY; i+= vertStep ) { 
        g.setColor( Color.GRAY ) ; 
        g.drawLine( HORIZ_OFFSET, getJavaY(i), getWidth() - HORIZ_OFFSET, getJavaY(i)); 
        g.setColor( Color.BLACK ) ;        
      ...
      g.drawLine(HORIZ_OFFSET, VERT_OFFSET, HORIZ_OFFSET, getHeight() - VERT_OFFSET ) ; 
      g.drawLine(HORIZ_OFFSET + 1, VERT_OFFSET, HORIZ_OFFSET + 1, getHeight() - VERT_OFFSET ) ;
      
      g.drawLine(HORIZ_OFFSET, getHeight() - VERT_OFFSET, getWidth() - HORIZ_OFFSET , getHeight() - VERT_OFFSET ) ; 
      g.drawLine(HORIZ_OFFSET, getHeight() - VERT_OFFSET + 1, getWidth() - HORIZ_OFFSET , getHeight() - VERT_OFFSET + 1) ;
      ...
      for ( int i = 0 ; i < completedLinePoints.size() - 1 ; i++ ) { 
        ...
        g2.drawLine(getJavaX((int)p1.getX()), getJavaY((int)p1.getY()), getJavaX((int)p2.getX()), getJavaY((int)p2.getY())); 
      }
    ...
    int roundTo1SigDigit(int num) {
      int fuckingLargest = (int)Math.log10(num);
      int fuckingDivisor = (int)Math.ceil( (double)num/Math.pow(10, fuckingLargest) ) ; 
      
      return fuckingDivisor * (int)Math.pow(10, fuckingLargest); 
    }
